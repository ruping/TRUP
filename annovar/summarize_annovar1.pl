...
#				}
				my @aachange = split (/,/, $aachange);
				if ($aachange[0] ne "UNKNOWN" ) {
					my @aachangestring;
					for my $i (0 .. @aachange-1) {
						my @eachaachange = split (/:/, $aachange[$i]);
						push @aachangestring, "$eachaachange[1]:$eachaachange[3]:$eachaachange[4]";
					}
					push @{$allstep[1]}, $varstring, $efun, join(",",@aachangestring);
					# print join(",",@aachangestring) ,"\n";
				} else {
				push @{$allstep[1]}, $varstring, $efun, $aachange[0];	 #aachange could be "UNKNOWN"
				}
			}
		}
	}
