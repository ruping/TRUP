...
while ( <IN> ){
   chomp;
   my ($id, $type, $chr, $coor, $support, $rep) = split /\t/;
   ...
   $bp_regions{$id}{'bps'} .= "$id\t$chr\t$coor\t$support\n";
...
foreach my $bp_id (sort {$a<=>$b} keys %bp_regions){
  my $chr1   = $bp_regions{$bp_id}{'chr1'};
  my $start1 = $bp_regions{$bp_id}{'start1'};
  my $end1   = $bp_regions{$bp_id}{'end1'};

  my $chr2   = $bp_regions{$bp_id}{'chr2'};
  my $start2 = $bp_regions{$bp_id}{'start2'};
  my $end2   = $bp_regions{$bp_id}{'end2'};

  my $seq1 = substr ($genome{$chr1}, $start1-1, $end1-$start1+1);
  my $seq2 = substr ($genome{$chr2}, $start2-1, $end2-$start2+1);

  #print "$bp_regions{$bp_id}{'bps'}";
