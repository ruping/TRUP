...
#	print $_;
	if ($_ =~ m/^Func/) 
	{ #Func    Gene    ExonicFunc      AAChange        Conserved       SegDup  ESP5400_ALL     1000g2012feb_ALL        dbSNP135        AVSIFT  LJB_PhyloP      LJB_PhyloP_Pred LJB_SIFT        LJB_SIFT_Pred   LJB_PolyPhen2   LJB_PolyPhen2_Pred  LJB_LRT  LJB_LRT_Pred    LRT_MutationTaster      LRT_MutationTaster_Pred LJB_GERP++      Chr     Start   End     Ref     Obs     Otherinfo
		print_annovar_header($_,*out);  ## headers from Func
		print out $chrline;
	...
	else
	{
		...
		$buffer="";
		my @item = split("\t");
		$buffer.="$item[21]\t$item[22]\t";	#Chr, Pos
		$buffer.=  ($item[8]) ? "$item[8]\t" : ".\t";	# ID
		$buffer.="$item[24]\t$item[25]\t$item[31]\t$item[32]\t";#Ref, Alt,Qual, Filter
		## Update INFO with annotations
		my $info="$item[33]";

		if ($item[6]) { $info.=";ESP5400=$item[6]";}
		...
		if ($item[10]) { $info.=";phylop.score=$item[10]";}
		if ($item[11]) { $info.=";phylop.pred=$item[11]";}
                if ($item[12]) { $info.=";sift.score=$item[12]";}
                if ($item[13]) { $info.=";sift.pred=$item[13]";}
	        if ($item[14]) { $info.=";pp2.score=$item[14]";}
                if ($item[15]) { $info.=";pp2.pred=$item[15]";}
		if ($item[16]) { $info.=";ljb_lrt.score=$item[16]";}
                if ($item[17]) { $info.=";ljb_lrt.pred=$item[17]";}
                if ($item[18]) { $info.=";lrt_mt.score=$item[18]";}
                if ($item[19]) { $info.=";lrt_mt.pred=$item[19]";}
